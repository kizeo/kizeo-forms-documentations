...
To do that operation, you have to send a `GET` request to : `https://forms.kizeo.com/rest/v3/forms/{formId}/exports`.
...
Both of them are based on a `POST` request to send to : `https://forms.kizeo.com/rest/v3/forms/{formId}/data/multiple/{format}` in which **`{format}`** can be **csv** or **excel**, depending on which format you want.
...
To do this action, you will have to send a `POST` request to : `https://forms.kizeo.com/rest/v3/forms/{formId}/multiple_data/exports/{exportId}/pdf` where you replace **`{formId}`** by the id of the data form, and **`{exportId}`** by the id of the chosen export.
...
### 4 - Export data to customized Word and Excel format
...
The both consist to send a `POST` request to : `https://forms.kizeo.com/rest/v3/forms/{formId}/data/multiple/{format}_custom` where **`{formId}`** is the id of the form where the data are and **`{format}`** can be **csv** or **excel**.
